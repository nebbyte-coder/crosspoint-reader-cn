...
// keep all roles ≤ 14 pt so the activity works in every build environment.
constexpr int kBigDigitFont = NOTOSERIF_14_FONT_ID;   // fixed (given) digits + palette numerals
...
constexpr int kHeroFont = NOTOSERIF_14_FONT_ID;       // win-screen "Solved!"
constexpr int kStatValueFont = NOTOSANS_14_FONT_ID;   // win-screen stat columns
...
void SudokuGameActivity::renderGenerating() {
  ...
  renderer.drawCenteredText(NOTOSANS_14_FONT_ID, h / 2 - 16, tr(STR_SUDOKU_GENERATING));
